                ...
                const charactersWithEpisodes = await Promise.all(
                    response.data.results.map(async character => {
                        ...
                        character.lastEpisode = episodes.reduce((latest, current) => 
                            new Date(current.air_date) > new Date(latest.air_date) ? current : latest
                        ).name;
                        return character;
                    })
                );
        ...
        async function searchCharacters() {
            ...
            fetchAndDisplayCharacters(searchTerm === '' ? 1 : searchTerm);
        }
        ...
        // Load characters from the first page when the page is loaded
